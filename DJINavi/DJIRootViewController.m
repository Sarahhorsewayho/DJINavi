...
-(void) initUI
{
    ...
    [self.gsButtonVC.view setCenter:CGPointMake(247, 638)];
...
- (void)focusMap
{
    self.selfLocation = CLLocationCoordinate2DMake(45.742536, 126.567933);
    if (CLLocationCoordinate2DIsValid(self.selfLocation)) {
        MKCoordinateRegion region = {0};
        region.center = self.selfLocation;
...
//- (void)focusMap
//{
//    //self.selfLocation = CLLocationCoordinate2DMake(31.141333, 121.661735);
//
//    if (CLLocationCoordinate2DIsValid(self.amapcoord)) {
//        MKCoordinateRegion region = {0};
//        //将GPS转成高德坐标
//        // CLLocationCoordinate2D amapcoord = AMapCoordinateConvert(CLLocationCoordinate2DMake(self.userLocation.latitude, self.userLocation.longitude), AMapCoordinateTypeGPS);
//        region.center = self.amapcoord;
//        region.span.latitudeDelta = 0.001;
//        region.span.longitudeDelta = 0.001;
//
//        [self.mapView setRegion:region animated:YES];
...
//    if(tapGesture.state == UIGestureRecognizerStateEnded){
//        if (self.isEditingPoints) {
//            [self.mapController addPoint:point withMapView:self.mapView withLocation:self.amapcoord];
...
- (void)finishBtnActionInDJIWaypointConfigViewController:(DJIWaypointConfigViewController *)waypointConfigVC
{
    ...
    [[self missionOperator] addListenerToFinished:self withQueue:dispatch_get_main_queue() andBlock:^(NSError * _Nullable error) {
        ...
        if (error) {
            ...
            [target showAlertViewWithTitle:@"导航结束" withMessage:nil];
        }
    }];
    ...
    [[self missionOperator] uploadMissionWithCompletion:^(NSError * _Nullable error) {
        if (error){
            ...
            ShowMessage(@"", @"设置成功", nil, @"好");
        }
    }];
    NSLog(@"===== is %ld", [self missionOperator].currentState);
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    CLLocation* location = [locations lastObject];
    self.userLocation = location.coordinate;
    self.amapcoord = AMapCoordinateConvert(CLLocationCoordinate2DMake(self.userLocation.latitude, self.userLocation.longitude), AMapCoordinateTypeGPS);
}
