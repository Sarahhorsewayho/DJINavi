...
/**
 *  Enables the setting to save original images when shooting panorama photos.
 *  CAUTION: enables it may consume 500 MB in the SD card or the internal storage
 *  for each panorama.
 *  
 *  @param enabled `YES` to enable the setting.
 *  @param completion Completion block that receives the setter result.
 */
- (void)setSavingPanoOriginalImagesEnabled:(BOOL)enabled withCompletion:(DJICompletionBlock)completion;


/**
 *  Determines whether the setting to save original images for panorama is enabled.
 *  CAUTION: when it is enabled, the camera may consume 500 MB in the SD card or the
 *  internal storage for each panorama.
 *  
 *  @param enabled `YES` if the camera will save original images for panorama.
 *  @param error Error retrieving the value.
 *  @param completion Completion block to receive the result.
 */
- (void)getSavingPanoOriginalImagesEnabledWithCompletion:(void (^_Nonnull)(BOOL enabled, NSError *_Nullable error))completion;
...
/**
 *  Sets the thermal window's postion relative to the visual window. The setting is
...
/**
 *  Gets the PIP position which determines the thermal window's postion relative to
